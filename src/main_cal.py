...
class SurfaceSpecies2:
    def __init__(self):
        ...
        self.sfinitial=None
    def add_surface(self, surfacename: str, surface_ms: str, sites, area, mass, c1, c2,sites_initial,c1_initial=1,c2_initial=1):
    ...
    def reset_cap(self,c1,c2): # reset for ccm to reduce parameters
        self.surface_C1=c1
        self.surface_C2=c2
    #
...
class Adsorption:
    ...
    def species_definition(self, database_path, otherspecies):
    ...
    def set_para(self,unknowns):
        ...
        for sp in self.surface:
            ...
            if isinstance(sp.surface_sites,tuple)==False:
                surface_amounts += "\n" + "            " + sp.surface_mp + "\t" + str(sp.surface_sites) + "\t" + str(sp.surface_area) + "\t" + str(sp.surface_mass)
            else:
                surface_amounts += "\n" + "            " + sp.surface_mp + "\t" +str(unknowns[n]) + "\t" + str(sp.surface_area) + "\t" + str(sp.surface_mass)
                n+=1
                ...
                if self.p_type=='CDMUSIC':
                    if sp.surface_reactions[react][2]==True:
                        if isinstance(sp.surface_reactions[react][1],tuple)==False:
                            surface_reaction +="\n" + "            " + '-cd_music  '+str(sp.surface_reactions[react][3]-sp.surface_reactions[react][1])+" "+str(sp.surface_reactions[react][1])+' 0'
                        else:
                            surface_reaction += "\n" + "            " + '-cd_music  ' + str(sp.surface_reactions[react][3] -unknowns[n]) + " " + str(unknowns[n]) + ' 0'
                            n+=1
                    else:
                        if isinstance(sp.surface_reactions[react][1],tuple)==False:
                            surface_reaction +="\n" + "            " + '-cd_music  '+'0 '+str(sp.surface_reactions[react][1])+" "+str(sp.surface_reactions[react][3]-sp.surface_reactions[react][1])
                        else:
                            surface_reaction += "\n" + "            " + '-cd_music  ' + '0 '+str(unknowns[n])+" "+str(sp.surface_reactions[react][3]-unknowns[n])
                            n+=1
...
def get_metal(ppp):
    ...
    for i in range(1, len(metal)):
        re_total_metal.append(metal[i][3])
...
def advanced_evaluation(exp_data, results, titration: Adsorption,mix=False,auto_p=False,eq=None,ph_list=None):
    ...
    # print(titration.total)
    if mix==False:
        model_res = run_phreeqc_ad(titration)
    else:
        model_res = run_phreeqc(titration)
    raw_r2 = r2(exp_data, model_res)
    adj_r2 = 1 - (1 - raw_r2) * (len(exp_data) - 1) / (len(exp_data) - 1 - len(p))
    rms = results.fun / len(exp_data) ** 0.5
    return raw_r2, adj_r2, rms, results.nfev, model_res,titration.sms
