...
#ifndef PHREEFIT_CDMUSIC_BROYDEN
#define PHREEFIT_CDMUSIC_BROYDEN 0
#endif

#ifndef PHREEFIT_CDMUSIC_ANALYTIC
#define PHREEFIT_CDMUSIC_ANALYTIC 0
#endif

#ifndef PHREEFIT_CDMUSIC_COLORING
#define PHREEFIT_CDMUSIC_COLORING 0
#endif

#ifndef PHREEFIT_CDMUSIC_LOCAL_ANALYTIC
#define PHREEFIT_CDMUSIC_LOCAL_ANALYTIC 1
#endif
 ...
 */
	...
	bool cd_music_jacobian_valid = false;
	size_t cd_music_jacobian_unknown_count = 0;
	int cd_music_jacobian_reuse_count = 0;
#if PHREEFIT_CDMUSIC_BROYDEN
	bool cd_music_broyden_pending = false;
	std::vector<LDBLE> cd_music_previous_residual;
	std::vector<LDBLE> cd_music_previous_step;
#endif
	std::vector<int> cd_music_column_colors;
	std::vector<std::vector<bool> > cd_music_jacobian_support;
#if PHREEFIT_CDMUSIC_COLORING == 1
	int cd_music_full_jacobian_builds = 0;
#endif
 ...
 */
			...
			bool cd_music_matrix_shape_valid =
				count_unknowns > 0 &&
				count_unknowns <= my_array.size() / (count_unknowns + 1);
#if PHREEFIT_CDMUSIC_BROYDEN
			bool broyden_updated = false;
			/*
			 * The matrix stored in my_array is -d(residual)/d(x), so its
			 * secant vector is residual_old - residual_new.  Apply a good
			 * Broyden rank-one update only after a successful step and while
			 * the equation/inequality structure is unchanged.  Periodic full
			 * rebuilds limit accumulated secant error.
			 */
			if (is_cd_music && cd_music_jacobian_valid &&
				cd_music_broyden_pending &&
				cd_music_jacobian_reuse_count < 1 &&
				remove_unstable_phases == FALSE &&
				cd_music_previous_residual.size() == (size_t) count_unknowns &&
				cd_music_previous_step.size() == (size_t) count_unknowns)
			{
				LDBLE denominator = 0.0;
				for (int column = 0; column < count_unknowns; column++)
				{
					denominator += cd_music_previous_step[column] *
						cd_music_previous_step[column];
				}
				if (denominator > 1e-24 && PHR_ISFINITE((double) denominator))
				{
					std::vector<LDBLE> correction(count_unknowns, 0.0);
					LDBLE largest_matrix_entry = 0.0;
					LDBLE largest_correction = 0.0;
					bool finite_update = true;
					for (int row = 0; row < count_unknowns; row++)
					{
						LDBLE matrix_step = 0.0;
						for (int column = 0; column < count_unknowns; column++)
						{
							LDBLE entry = my_array[(size_t) row *
								(count_unknowns + 1) + (size_t) column];
							matrix_step += entry * cd_music_previous_step[column];
							largest_matrix_entry =
								std::max(largest_matrix_entry, (LDBLE) fabs(entry));
						}
						LDBLE secant = cd_music_previous_residual[row] - residual[row];
						correction[row] = (secant - matrix_step) / denominator;
						largest_correction =
							std::max(largest_correction, (LDBLE) fabs(correction[row]));
						if (!PHR_ISFINITE((double) correction[row]))
						{
							finite_update = false;
							break;
						}
					}
					/* Reject a numerically explosive update and rebuild instead. */
					LDBLE largest_step = 0.0;
					for (int column = 0; column < count_unknowns; column++)
					{
						largest_step = std::max(largest_step,
							(LDBLE) fabs(cd_music_previous_step[column]));
					}
					if (finite_update && largest_step > 1e-12 &&
						largest_correction * largest_step <=
							100.0 * std::max((LDBLE) 1.0, largest_matrix_entry))
					{
						for (int row = 0; row < count_unknowns; row++)
						{
							for (int column = 0; column < count_unknowns; column++)
							{
								my_array[(size_t) row * (count_unknowns + 1) +
									(size_t) column] += correction[row] *
									cd_music_previous_step[column];
							}
						}
						broyden_updated = true;
						cd_music_jacobian_reuse_count++;
					}
				}
			}
			cd_music_broyden_pending = false;
#endif
			// Modified Newton for CD-MUSIC: reuse a complete numerical
			// Jacobian for one Newton step, then rebuild it. Reuse is abandoned
			// immediately when the equation set or inequality solve changes.
#if PHREEFIT_CDMUSIC_BROYDEN
			bool reuse_cd_music_jacobian = broyden_updated;
#else
			bool reuse_cd_music_jacobian =
				cd_music_reuse_supported && cd_music_jacobian_valid &&
				cd_music_matrix_shape_valid &&
				cd_music_jacobian_unknown_count == count_unknowns &&
				cd_music_jacobian_reuse_count < 1 &&
				remove_unstable_phases == FALSE;
#endif

			if (!reuse_cd_music_jacobian)
			{
				bool use_cd_music_coloring = false;
#if PHREEFIT_CDMUSIC_COLORING == 1
				/* Learn from a full matrix and color only every other rebuild. */
				use_cd_music_coloring = is_cd_music &&
					!cd_music_column_colors.empty() &&
					(cd_music_full_jacobian_builds % 2) == 1 &&
					remove_unstable_phases == FALSE;
#endif
				if (state >= REACTION && numerical_deriv)
				{
					//jacobian_sums();
					numerical_jacobian(
						use_cd_music_coloring ? &cd_music_column_colors : NULL,
						use_cd_music_coloring ? &cd_music_jacobian_support : NULL);
				}
				else /* hmm */
				{
					jacobian_sums();
...
#if PHREEFIT_CDMUSIC_COLORING == 1
				if (is_cd_music && !use_cd_music_coloring)
				{
					cd_music_jacobian_support.assign(count_unknowns,
						std::vector<bool>(count_unknowns, false));
					std::vector<LDBLE> row_scale(count_unknowns, 0.0);
					for (int row = 0; row < count_unknowns; row++)
					{
						for (int column = 0; column < count_unknowns; column++)
						{
							row_scale[row] = std::max(row_scale[row],
								(LDBLE) fabs(my_array[(size_t) row *
									(count_unknowns + 1) + (size_t) column]));
						}
					}
					for (int row = 0; row < count_unknowns; row++)
					{
						LDBLE threshold = std::max((LDBLE) 1e-16,
							row_scale[row] * (LDBLE) 1e-12);
						for (int column = 0; column < count_unknowns; column++)
						{
							LDBLE entry = my_array[(size_t) row *
								(count_unknowns + 1) + (size_t) column];
							cd_music_jacobian_support[row][column] =
								fabs(entry) > threshold || row == column;
						}
					}
					cd_music_column_colors.assign(count_unknowns, -1);
					int color_count = 0;
					for (int column = 0; column < count_unknowns; column++)
					{
						std::vector<bool> unavailable(count_unknowns, false);
						for (int previous = 0; previous < column; previous++)
						{
							bool conflicts = false;
							for (int row = 0; row < count_unknowns; row++)
							{
								if (cd_music_jacobian_support[row][column] &&
									cd_music_jacobian_support[row][previous])
								{
									conflicts = true;
									break;
								}
							...
							if (conflicts)
							{
								unavailable[cd_music_column_colors[previous]] = true;
							}
						}
						int color = 0;
						while (color < color_count && unavailable[color])
						{
							color++;
						}
						...
						cd_music_column_colors[column] = color;
					}
				}
				if (is_cd_music)
				{
					cd_music_full_jacobian_builds++;
				}
#endif
				cd_music_jacobian_valid = cd_music_reuse_supported &&
					cd_music_matrix_shape_valid;
				cd_music_jacobian_unknown_count =
					cd_music_jacobian_valid ? count_unknowns : 0;
				cd_music_jacobian_reuse_count = 0;
			}
#if !PHREEFIT_CDMUSIC_BROYDEN
			else
			{
 ...
 */
			...
			{
#if PHREEFIT_CDMUSIC_BROYDEN
				std::vector<LDBLE> residual_before_step;
				if (is_cd_music)
				{
					residual_before_step.assign(residual.begin(), residual.end());
				}
#endif
				return_kode = ineq(l_kode);
				if (return_kode != OK)
				{
					cd_music_jacobian_valid = false;
#if PHREEFIT_CDMUSIC_BROYDEN
					cd_music_broyden_pending = false;
#endif
					if (debug_model == TRUE)
				...
				reset();
#if PHREEFIT_CDMUSIC_BROYDEN
				if (is_cd_music && return_kode == OK &&
					cd_music_jacobian_valid)
				{
					cd_music_previous_residual.swap(residual_before_step);
					cd_music_previous_step.assign(delta.begin(),
						delta.begin() + count_unknowns);
					/*
					 * numerical_jacobian perturbs these unknowns in log10
					 * activity, whereas reset receives a natural-log delta.
					 * Convert the accepted step to the coordinate used by the
					 * finite-difference columns before applying the secant update.
					 */
					for (int column = 0; column < count_unknowns; column++)
					{
						...
						case MH:
						case AH2O:
							cd_music_previous_step[column] /= LOG_10;
							break;
						default:
							break;
						}
					}
					cd_music_broyden_pending = true;
				}
#endif
			}
			gammas(mu_x);
 ...
 */
	if (surface_unknown != NULL && dl_type_x == cxxSurface::NO_DL)
	{
		if (use.Get_surface_ptr()->Get_type() == cxxSurface::CD_MUSIC)
		{
#if PHREEFIT_CDMUSIC_ANALYTIC
			/*
			 * The generic summation lists already contain derivatives of the
			 * charge assigned to each CD-MUSIC plane.  Combine those charge
			 * rows into the three capacitance residuals.  Difficult columns
			 * (surface potentials, ionic strength, and water) are overwritten
			 * by the hybrid numerical pass in numerical_jacobian.
			 */
			bool hybrid_supported =
				use.Get_surface_ptr()->Get_dl_type() == cxxSurface::NO_DL &&
				use.Get_gas_phase_ptr() == NULL &&
				use.Get_ss_assemblage_ptr() == NULL &&
				pure_phase_unknown == NULL && numerical_deriv == FALSE;
			if (hybrid_supported)
			{
				for (i = 0; i < count_unknowns; i++)
				{
					if (x[i]->type != SURFACE_CB || i + 2 >= count_unknowns ||
						x[i + 1]->type != SURFACE_CB1 ||
						x[i + 2]->type != SURFACE_CB2)
					{
						continue;
					}
					cxxSurfaceCharge *charge_ptr =
						use.Get_surface_ptr()->Find_charge(x[i]->surface_charge);
					if (charge_ptr == NULL || charge_ptr->Get_grams() <= 0)
					{
						continue;
					}
					std::vector<LDBLE> plane0(count_unknowns);
					std::vector<LDBLE> plane1(count_unknowns);
					std::vector<LDBLE> plane2(count_unknowns);
					for (j = 0; j < count_unknowns; j++)
					{
						plane0[j] = my_array[(size_t) i *
							(count_unknowns + 1) + (size_t) j];
						plane1[j] = my_array[(size_t) (i + 1) *
							(count_unknowns + 1) + (size_t) j];
						plane2[j] = my_array[(size_t) (i + 2) *
							(count_unknowns + 1) + (size_t) j];
					}
					LDBLE charge_scale = F_C_MOL /
						(charge_ptr->Get_specific_area() * charge_ptr->Get_grams());
					class master *psi2_master = surface_get_psi_master(
						charge_ptr->Get_name().c_str(), SURF_PSI2);
					LDBLE u = psi2_master->s->la * LOG_10;
					LDBLE ionic_sum = 0.0;
					LDBLE charge_sum = 0.0;
					for (size_t species_index = 0; species_index < s_x.size(); species_index++)
					{
						class species *species_ptr = s_x[species_index];
						if (species_ptr->type >= H2O)
						{
							continue;
						}
						LDBLE molality = under(species_ptr->lm);
						ionic_sum += molality *
							(exp(species_ptr->z * u) - 1.0);
						charge_sum += molality * species_ptr->z;
					}
					LDBLE charge_sign = charge_sum >= 0.0 ? 1.0 : -1.0;
					ionic_sum += fabs(charge_sum) *
						(exp(-charge_sign * u) - 1.0);
					LDBLE sinh_factor = sqrt(8 * eps_r * EPSILON_ZERO *
						(R_KJ_DEG_MOL * 1000) * tk_x * 1000);
					for (j = 0; j < count_unknowns; j++)
					{
						LDBLE diffuse_derivative = 0.0;
						bool numerical_column =
							x[j]->type == SURFACE_CB ||
							x[j]->type == SURFACE_CB1 ||
							x[j]->type == SURFACE_CB2 ||
							x[j]->type == MU || x[j]->type == AH2O ||
							x[j]->type == MH2O;
						if (!numerical_column && ionic_sum > 1e-30)
						{
							LDBLE d_ionic_sum = 0.0;
							LDBLE d_charge_sum = 0.0;
							for (size_t species_index = 0;
								species_index < s_x.size(); species_index++)
							{
								class species *species_ptr = s_x[species_index];
								if (species_ptr->type >= H2O)
								{
								...
								LDBLE d_log_molality = 0.0;
								for (class rxn_token *token =
									&species_ptr->rxn_x.token[0] + 1;
									token->s != NULL; token++)
								{
									class master *master_ptr = NULL;
									if (token->s->secondary != NULL &&
										token->s->secondary->in == TRUE)
									{
										master_ptr = token->s->secondary;
									}
									else
									{
										master_ptr = token->s->primary;
									}
									if (master_ptr != NULL && master_ptr->unknown != NULL &&
										master_ptr->unknown->number == j)
									{
										d_log_molality += token->coef;
									}
								}
								LDBLE d_molality = under(species_ptr->lm) *
									d_log_molality;
								d_ionic_sum += d_molality *
									(exp(species_ptr->z * u) - 1.0);
								d_charge_sum += d_molality * species_ptr->z;
							}
							d_ionic_sum += charge_sign * d_charge_sum *
								(exp(-charge_sign * u) - 1.0);
							LDBLE sigma_sign = u < 0.0 ? -1.0 : 1.0;
							diffuse_derivative = sigma_sign * 0.25 * sinh_factor *
								d_ionic_sum / sqrt(ionic_sum);
						}
						my_array[(size_t) i * (count_unknowns + 1) + (size_t) j] =
							-charge_scale * plane0[j];
						my_array[(size_t) (i + 1) * (count_unknowns + 1) +
							(size_t) j] = -charge_scale * (plane0[j] + plane1[j]);
						my_array[(size_t) (i + 2) * (count_unknowns + 1) +
							(size_t) j] = -charge_scale *
							(plane0[j] + plane1[j] + plane2[j]) -
							diffuse_derivative;
					}
				}
				return (OK);
...
#endif
			/* Unsupported CD-MUSIC variants retain the original full
			 * numerical-Jacobian path; these values are overwritten there. */
		}
		else if (use.Get_surface_ptr()->Get_type() != cxxSurface::CCM)
		{
			...
			for (i = 0; i < count_unknowns; i++)
			{
				...
				if (x[i]->type == SURFACE_CB && charge_ptr != NULL &&
...
/* ---------------------------------------------------------------------- */
bool Phreeqc::
build_cd_music_local_analytic_columns(
	const std::vector<LDBLE> &seed,
	std::vector<LDBLE> &analytic,
	std::vector<bool> &analytic_columns)
/* ---------------------------------------------------------------------- */
{
	const size_t matrix_width = count_unknowns + 1;
	if (count_unknowns == 0 ||
		count_unknowns == (std::numeric_limits<size_t>::max)() ||
		count_unknowns > (size_t) (std::numeric_limits<int>::max)() ||
		count_unknowns > (std::numeric_limits<size_t>::max)() / matrix_width)
	{
		return false;
	}
	const size_t required_matrix_size = count_unknowns * matrix_width;
	if (use.Get_surface_ptr() == NULL ||
		use.Get_surface_ptr()->Get_type() != cxxSurface::CD_MUSIC ||
		use.Get_surface_ptr()->Get_dl_type() != cxxSurface::NO_DL ||
		x.size() < count_unknowns ||
		seed.size() != my_array.size() ||
		seed.size() < required_matrix_size)
	{
		return false;
	}
	analytic.assign(my_array.size(), 0.0);
	analytic_columns.assign(count_unknowns, false);
	bool found = false;
	for (int row0 = 0; row0 + 2 < count_unknowns; row0++)
	{
		if (x[row0] == NULL || x[row0 + 1] == NULL || x[row0 + 2] == NULL ||
			x[row0]->type != SURFACE_CB ||
			x[row0 + 1]->type != SURFACE_CB1 ||
			x[row0 + 2]->type != SURFACE_CB2)
		{
			continue;
		}
		cxxSurfaceCharge *charge_ptr =
			use.Get_surface_ptr()->Find_charge(x[row0]->surface_charge);
		if (charge_ptr == NULL || charge_ptr->Get_grams() <= 0.0 ||
			charge_ptr->Get_specific_area() <= 0.0)
		{
			continue;
		}
		found = true;
		LDBLE scale = F_C_MOL /
			(charge_ptr->Get_specific_area() * charge_ptr->Get_grams());
		LDBLE beta = LOG_10 * R_KJ_DEG_MOL * tk_x / F_KJ_V_EQ;
		LDBLE diffuse_derivative = 0.0;
		class master *psi2_master = surface_get_psi_master(
			charge_ptr->Get_name().c_str(), SURF_PSI2);
		if (psi2_master == NULL || psi2_master->s == NULL)
		{
			return false;
		}
		LDBLE u = psi2_master->s->la * LOG_10;
		LDBLE ionic_sum = 0.0;
		LDBLE ionic_derivative = 0.0;
		LDBLE charge_sum = 0.0;
		LDBLE second_moment = 0.0;
		for (size_t species_index = 0; species_index < s_x.size(); species_index++)
		{
			class species *species_ptr = s_x[species_index];
			if (species_ptr == NULL)
			...
			if (species_ptr->type >= H2O) continue;
			LDBLE molality = under(species_ptr->lm);
			LDBLE exponential = exp(species_ptr->z * u);
			ionic_sum += molality * (exponential - 1.0);
			ionic_derivative += molality * species_ptr->z * exponential;
			charge_sum += molality * species_ptr->z;
			second_moment += molality * species_ptr->z * species_ptr->z;
		}
		LDBLE charge_sign = charge_sum >= 0.0 ? 1.0 : -1.0;
		ionic_sum += fabs(charge_sum) * (exp(-charge_sign * u) - 1.0);
		ionic_derivative -= fabs(charge_sum) * charge_sign *
			exp(-charge_sign * u);
		LDBLE sinh_factor = sqrt(8 * eps_r * EPSILON_ZERO *
			(R_KJ_DEG_MOL * 1000) * tk_x * 1000);
		if (fabs(u) <= 1e-8)
		{
			second_moment += fabs(charge_sum);
			diffuse_derivative = 0.5 * sinh_factor * LOG_10 *
				sqrt(0.5 * second_moment);
		}
		else if (fabs(ionic_sum) > 1e-30)
		{
			LDBLE potential_sign = u < 0.0 ? -1.0 : 1.0;
			LDBLE sum_sign = ionic_sum < 0.0 ? -1.0 : 1.0;
			diffuse_derivative = potential_sign * 0.25 * sinh_factor *
				sum_sign * LOG_10 * ionic_derivative / sqrt(fabs(ionic_sum));
		}
		else
		{
			return false;
		}
		if (!PHR_ISFINITE((double) diffuse_derivative))
		{
			return false;
		}
		...
		{
			int column = row0 + local_column;
			analytic_columns[column] = true;
			for (int row = 0; row < count_unknowns; row++)
			{
				analytic[(size_t) row * (count_unknowns + 1) + (size_t) column] =
					LOG_10 * seed[(size_t) row *
						(count_unknowns + 1) + (size_t) column];
			}
			LDBLE plane0 = seed[(size_t) row0 *
				(count_unknowns + 1) + (size_t) column];
			LDBLE plane1 = seed[(size_t) (row0 + 1) *
				(count_unknowns + 1) + (size_t) column];
			LDBLE plane2 = seed[(size_t) (row0 + 2) *
				(count_unknowns + 1) + (size_t) column];
			LDBLE value0 = -scale * LOG_10 * plane0;
			if (local_column == 0) value0 -= charge_ptr->Get_capacitance0() * beta;
			if (local_column == 1) value0 += charge_ptr->Get_capacitance0() * beta;
			LDBLE value1 = -scale * LOG_10 * (plane0 + plane1);
			if (local_column == 1) value1 -= charge_ptr->Get_capacitance1() * beta;
			if (local_column == 2) value1 += charge_ptr->Get_capacitance1() * beta;
			LDBLE value2 = -scale * LOG_10 * (plane0 + plane1 + plane2);
			if (local_column == 2) value2 -= diffuse_derivative;
			if (!PHR_ISFINITE((double) value0) ||
				!PHR_ISFINITE((double) value1) ||
				!PHR_ISFINITE((double) value2))
			{
				return false;
			}
			analytic[(size_t) row0 * (count_unknowns + 1) + (size_t) column] = value0;
			analytic[(size_t) (row0 + 1) * (count_unknowns + 1) + (size_t) column] = value1;
			analytic[(size_t) (row0 + 2) * (count_unknowns + 1) + (size_t) column] = value2;
		}
	}
	return found;
}
...
/* ---------------------------------------------------------------------- */
int Phreeqc::
numerical_jacobian(const std::vector<int> *column_colors,
	const std::vector<std::vector<bool> > *jacobian_support)
/* ---------------------------------------------------------------------- */
{
 ...
 */
	...
	d2 = 0;
#if PHREEFIT_CDMUSIC_COLORING == 1
	bool use_coloring = column_colors != NULL && jacobian_support != NULL &&
		column_colors->size() == (size_t) count_unknowns &&
		jacobian_support->size() == (size_t) count_unknowns &&
		use.Get_surface_ptr() != NULL &&
		use.Get_surface_ptr()->Get_type() == cxxSurface::CD_MUSIC &&
		use.Get_surface_ptr()->Get_dl_type() == cxxSurface::NO_DL &&
		gas_phase_ptr == NULL && use.Get_ss_assemblage_ptr() == NULL &&
		pure_phase_unknown == NULL && numerical_deriv == FALSE;
	if (use_coloring)
	{
		int color_count = 0;
		for (i = 0; i < count_unknowns; i++)
		{
			color_count = std::max(color_count, (*column_colors)[i] + 1);
			switch (x[i]->type)
			{
			case MB: case ALK: case CB: case SOLUTION_PHASE_BOUNDARY:
			case EXCH: case SURFACE: case SURFACE_CB: case SURFACE_CB1:
			case SURFACE_CB2: case MH: case AH2O: case MH2O: case MU:
			case PITZER_GAMMA:
				break;
			default:
				use_coloring = false;
	...
	if (use_coloring)
	{
		std::vector<LDBLE> column_step(count_unknowns, 0.0);
		int color_count = 0;
		for (i = 0; i < count_unknowns; i++)
		{
			color_count = std::max(color_count, (*column_colors)[i] + 1);
		...
		{
			bool mu_perturbed = false;
			for (i = 0; i < count_unknowns; i++)
			{
				if ((*column_colors)[i] != color)
				{
					continue;
				}
				switch (x[i]->type)
				...
				case PITZER_GAMMA:
					...
					break;
				case MH2O:
					column_step[i] = mass_water_aq_x * d;
					mass_water_aq_x += column_step[i];
					x[i]->master[0]->s->moles = mass_water_aq_x / gfw_water;
					break;
				case MU:
					column_step[i] = d * mu_x;
					mu_x += column_step[i];
					mu_perturbed = true;
					break;
				}
			...
			for (i = 0; i < count_unknowns; i++)
			{
				...
				for (j = 0; j < count_unknowns; j++)
				{
					LDBLE &entry = my_array[(size_t) j *
						(count_unknowns + 1) + (size_t) i];
					entry = 0.0;
					if ((*jacobian_support)[j][i])
					{
						entry = -(residual[j] - base[j]) / column_step[i];
						if (x[i]->type == MH2O)
						{
							entry *= mass_water_aq_x;
						}
					}
				}
			}
			for (i = 0; i < count_unknowns; i++)
			{
				...
				case EXCH: case SURFACE: case SURFACE_CB: case SURFACE_CB1:
				case SURFACE_CB2: case AH2O:
					x[i]->master[0]->s->la -= column_step[i];
					break;
				case MH:
					s_eminus->la -= column_step[i];
					break;
				case PITZER_GAMMA:
					x[i]->s->lg -= column_step[i];
					break;
				case MH2O:
					mass_water_aq_x -= column_step[i];
					x[i]->master[0]->s->moles = mass_water_aq_x / gfw_water;
...
/* ---------------------------------------------------------------------- */
{
	...
	bool hybrid_cd_music = false;
	std::vector<LDBLE> analytic_cd_music_matrix;
	std::vector<LDBLE> local_analytic_seed;
	std::vector<LDBLE> local_analytic_values;
	std::vector<bool> local_analytic_columns;
	bool local_analytic_ready = false;
#if PHREEFIT_CDMUSIC_ANALYTIC
	hybrid_cd_music =
		use.Get_surface_ptr() != NULL &&
		use.Get_surface_ptr()->Get_type() == cxxSurface::CD_MUSIC &&
	...
	if (hybrid_cd_music)
	{
		analytic_cd_music_matrix.assign(my_array.begin(), my_array.end());
	}
#if PHREEFIT_CDMUSIC_LOCAL_ANALYTIC
	if (use.Get_surface_ptr() != NULL &&
		use.Get_surface_ptr()->Get_type() == cxxSurface::CD_MUSIC &&
		use.Get_surface_ptr()->Get_dl_type() == cxxSurface::NO_DL &&
		gas_phase_ptr == NULL && use.Get_ss_assemblage_ptr() == NULL &&
		...
		numerical_deriv == FALSE)
	{
		local_analytic_seed.assign(my_array.begin(), my_array.end());
		local_analytic_ready = build_cd_music_local_analytic_columns(
			local_analytic_seed, local_analytic_values, local_analytic_columns);
	}
#endif
	...
	d2 = 0;
#if PHREEFIT_CDMUSIC_COLORING == 1
	bool grouped_cd_music = false;
	grouped_cd_music = column_colors != NULL && jacobian_support != NULL &&
		column_colors->size() == (size_t) count_unknowns &&
		jacobian_support->size() == (size_t) count_unknowns &&
		use.Get_surface_ptr() != NULL &&
		use.Get_surface_ptr()->Get_type() == cxxSurface::CD_MUSIC &&
		...
		gas_phase_ptr == NULL && use.Get_ss_assemblage_ptr() == NULL &&
		pure_phase_unknown == NULL && numerical_deriv == FALSE;
	int grouped_color_count = 0;
	for (i = 0; grouped_cd_music && i < count_unknowns; i++)
	{
		grouped_color_count = std::max(grouped_color_count,
			(*column_colors)[i] + 1);
		switch (x[i]->type)
		{
		case MB: case ALK: case CB: case SOLUTION_PHASE_BOUNDARY:
		...
		case PITZER_GAMMA:
			break;
		default:
			grouped_cd_music = false;
			break;
		}
	}
	if (grouped_cd_music && grouped_color_count > 0)
	{
		std::vector<LDBLE> grouped_step(count_unknowns, 0.0);
		for (int color = 0; color < grouped_color_count; color++)
		{
			bool grouped_mu = false;
			for (i = 0; i < count_unknowns; i++)
			{
				if ((*column_colors)[i] != color) continue;
				switch (x[i]->type)
				{
				case MB: case ALK: case CB: case SOLUTION_PHASE_BOUNDARY:
				case EXCH: case SURFACE: case SURFACE_CB: case SURFACE_CB1:
				case SURFACE_CB2: case AH2O:
					x[i]->master[0]->s->la += d; grouped_step[i] = d; break;
				case MH:
					s_eminus->la += d; grouped_step[i] = d; break;
				case PITZER_GAMMA:
					x[i]->s->lg += d; grouped_step[i] = d; break;
				case MH2O:
					grouped_step[i] = mass_water_aq_x * d;
					mass_water_aq_x += grouped_step[i];
					x[i]->master[0]->s->moles = mass_water_aq_x / gfw_water;
					break;
				case MU:
					grouped_step[i] = d * mu_x; mu_x += grouped_step[i];
					grouped_mu = true; break;
				}
			}
			if (grouped_mu) gammas(mu_x);
			molalities(TRUE);
			mb_sums();
			residuals();
			for (i = 0; i < count_unknowns; i++)
			{
				...
				for (j = 0; j < count_unknowns; j++)
				{
					...
					if ((*jacobian_support)[j][i])
					{
						entry = -(residual[j] - base[j]) / grouped_step[i];
						if (x[i]->type == MH2O) entry *= mass_water_aq_x;
					}
				}
			}
			for (i = 0; i < count_unknowns; i++)
			{
				...
				case EXCH: case SURFACE: case SURFACE_CB: case SURFACE_CB1:
				case SURFACE_CB2: case AH2O:
					x[i]->master[0]->s->la -= grouped_step[i]; break;
				case MH: s_eminus->la -= grouped_step[i]; break;
				case PITZER_GAMMA: x[i]->s->lg -= grouped_step[i]; break;
				case MH2O:
					mass_water_aq_x -= grouped_step[i];
					x[i]->master[0]->s->moles = mass_water_aq_x / gfw_water;
					break;
				case MU: mu_x -= grouped_step[i]; break;
				}
			}
			*use.Get_surface_ptr() = base_surface;
			if (grouped_mu) gammas(mu_x);
	...
	for (i = 0; i < count_unknowns; i++)
	{
#if PHREEFIT_CDMUSIC_LOCAL_ANALYTIC == 1
		if (local_analytic_ready && local_analytic_columns[i])
		{
			for (j = 0; j < count_unknowns; j++)
			{
				my_array[(size_t) j * (count_unknowns + 1) + (size_t) i] =
					local_analytic_values[(size_t) j *
						(count_unknowns + 1) + (size_t) i];
			}
			continue;
		}
#endif
		if (hybrid_cd_music && PHREEFIT_CDMUSIC_ANALYTIC != 2 &&
			x[i]->type != SURFACE_CB && x[i]->type != SURFACE_CB1 &&
			x[i]->type != SURFACE_CB2 && x[i]->type != MU &&
			x[i]->type != AH2O && x[i]->type != MH2O)
		{
			continue;
		}
		switch (x[i]->type)
		...
		// Activity coefficients depend on ionic strength (MU), not on the
		// other perturbed unknowns. Recomputing them for every finite-
		// difference column is both redundant and particularly expensive for
		// CD-MUSIC, which always uses this numerical Jacobian.
		if (x[i]->type == MU)
		{
			gammas(mu_x);
		}
	...
	//mb_ss();
	residuals();
#if PHREEFIT_CDMUSIC_LOCAL_ANALYTIC == 2
	static bool reported_cd_music_local_jacobian = false;
	if (!reported_cd_music_local_jacobian && !local_analytic_seed.empty())
	{
		reported_cd_music_local_jacobian = true;
		for (int row0 = 0; row0 + 2 < count_unknowns; row0++)
		{
			if (x[row0]->type != SURFACE_CB ||
			x[row0 + 1]->type != SURFACE_CB1 ||
			x[row0 + 2]->type != SURFACE_CB2)
			{
				continue;
			...
			ionic_derivative -= fabs(charge_sum) * charge_sign *
				exp(-charge_sign * u);
			if (fabs(ionic_sum) > 1e-30 && fabs(u) > 1e-14)
			{
				LDBLE sinh_factor = sqrt(8 * eps_r * EPSILON_ZERO *
					(R_KJ_DEG_MOL * 1000) * tk_x * 1000);
				LDBLE potential_sign = u < 0.0 ? -1.0 : 1.0;
			...
			else if (fabs(u) <= 1e-14)
			{
				/* Smooth u -> 0 limit after the fictitious counter-ion
				 * cancels the first-order charge term. */
				LDBLE sinh_factor = sqrt(8 * eps_r * EPSILON_ZERO *
					(R_KJ_DEG_MOL * 1000) * tk_x * 1000);
				second_moment += fabs(charge_sum);
				diffuse_derivative = 0.5 * sinh_factor * LOG_10 *
			...
			for (int local_row = 0; local_row < 3; local_row++)
			{
				...
				{
					int column = row0 + local_column;
					LDBLE plane0 = local_analytic_seed[(size_t) row0 *
						(count_unknowns + 1) + (size_t) column];
					LDBLE plane1 = local_analytic_seed[(size_t) (row0 + 1) *
						(count_unknowns + 1) + (size_t) column];
					LDBLE plane2 = local_analytic_seed[(size_t) (row0 + 2) *
						(count_unknowns + 1) + (size_t) column];
					LDBLE predicted = 0.0;
					if (local_row == 0)
					{
						predicted = -scale * LOG_10 * plane0;
						if (local_column == 0) predicted -= charge_ptr->Get_capacitance0() * beta;
						if (local_column == 1) predicted += charge_ptr->Get_capacitance0() * beta;
					}
					else if (local_row == 1)
					{
						predicted = -scale * LOG_10 * (plane0 + plane1);
						if (local_column == 1) predicted -= charge_ptr->Get_capacitance1() * beta;
						if (local_column == 2) predicted += charge_ptr->Get_capacitance1() * beta;
					}
					else
					{
						predicted = -scale * LOG_10 * (plane0 + plane1 + plane2);
						if (local_column == 2) predicted -= diffuse_derivative;
					}
					LDBLE numerical = my_array[(size_t) (row0 + local_row) *
						(count_unknowns + 1) + (size_t) column];
					LDBLE relative = fabs(predicted - numerical) /
						std::max((LDBLE) 1e-30,
							std::max((LDBLE) fabs(predicted), (LDBLE) fabs(numerical)));
					fprintf(stderr,
						"JLOCAL row=%d col=%d predicted=%.17g numerical=%.17g rel=%.6g raw=(%.17g,%.17g,%.17g)\n",
						local_row, local_column, (double) predicted,
						(double) numerical, (double) relative,
						(double) plane0, (double) plane1, (double) plane2);
				}
			}
			for (int local_column = 0; local_column < 3; local_column++)
			{
				...
				for (int row = 0; row < count_unknowns; row++)
				{
					if (row >= row0 && row <= row0 + 2) continue;
					LDBLE predicted = LOG_10 * local_analytic_seed[(size_t) row *
						(count_unknowns + 1) + (size_t) column];
					LDBLE numerical = my_array[(size_t) row *
						(count_unknowns + 1) + (size_t) column];
					LDBLE relative = fabs(predicted - numerical) /
						std::max((LDBLE) 1e-30,
							std::max((LDBLE) fabs(predicted), (LDBLE) fabs(numerical)));
					if (relative > 1e-3 && fabs(predicted - numerical) > 1e-12)
					{
						fprintf(stderr,
							"JCOLUMN row=%d type=%d col=%d predicted=%.17g numerical=%.17g rel=%.6g raw=%.17g\n",
							row, (int) x[row]->type, local_column,
							(double) predicted, (double) numerical,
							(double) relative,
							(double) local_analytic_seed[(size_t) row *
								(count_unknowns + 1) + (size_t) column]);
					}
				}
			}
...
#endif
#if PHREEFIT_CDMUSIC_ANALYTIC == 2
	static bool reported_cd_music_jacobian_difference = false;
	if (hybrid_cd_music && !reported_cd_music_jacobian_difference &&
		analytic_cd_music_matrix.size() == my_array.size())
	{
		reported_cd_music_jacobian_difference = true;
		for (int row = 0; row < count_unknowns; row++)
		{
			for (int column = 0; column < count_unknowns; column++)
			{
				LDBLE analytic_value = analytic_cd_music_matrix[(size_t) row *
					(count_unknowns + 1) + (size_t) column];
				LDBLE numerical_value = my_array[(size_t) row *
					(count_unknowns + 1) + (size_t) column];
				LDBLE scale = std::max((LDBLE) 1e-20,
					std::max((LDBLE) fabs(analytic_value),
						(LDBLE) fabs(numerical_value)));
				if (fabs(analytic_value - numerical_value) / scale > 1e-3 &&
					fabs(analytic_value - numerical_value) > 1e-14)
				{
					fprintf(stderr, "JDIFF row=%d type=%d col=%d type=%d a=%.17g n=%.17g rel=%.6g\\n",
						row, (int) x[row]->type, column, (int) x[column]->type,
						(double) analytic_value, (double) numerical_value,
						(double) (fabs(analytic_value - numerical_value) / scale));
				}
			}
		}
	}
#endif
#if PHREEFIT_CDMUSIC_COLORING == 2
	/*
	 * Diagnostic only: estimate the minimum number of finite-difference
	 * groups allowed by the measured Jacobian support.  Columns may share a
	 * color only when they do not affect any common residual row.
	 */
	static bool reported_cd_music_coloring = false;
	if (use.Get_surface_ptr() != NULL &&
		use.Get_surface_ptr()->Get_type() == cxxSurface::CD_MUSIC &&
		!reported_cd_music_coloring)
	{
		reported_cd_music_coloring = true;
		std::vector<LDBLE> row_scale(count_unknowns, 0.0);
		for (int row = 0; row < count_unknowns; row++)
		{
			for (int column = 0; column < count_unknowns; column++)
		...
		std::vector<int> colors(count_unknowns, -1);
		int color_count = 0;
		int nonzeros = 0;
		for (int column = 0; column < count_unknowns; column++)
		{
			...
			{
				LDBLE threshold = std::max((LDBLE) 1e-16,
					row_scale[row] * (LDBLE) 1e-10);
				LDBLE entry = my_array[(size_t) row *
					(count_unknowns + 1) + (size_t) column];
				if (fabs(entry) <= threshold)
				{
					continue;
				}
				nonzeros++;
				for (int previous = 0; previous < column; previous++)
				{
					...
					LDBLE previous_entry = my_array[(size_t) row *
						(count_unknowns + 1) + (size_t) previous];
					if (fabs(previous_entry) > threshold)
					{
						unavailable[colors[previous]] = true;
					}
				}
			}
			int color = 0;
		...
		fprintf(stderr,
			"JCOLOR columns=%d colors=%d nonzeros=%d density=%.6g\\n",
			count_unknowns, color_count, nonzeros,
			(double) nonzeros /
				(double) (count_unknowns * count_unknowns));
	}
#endif
	//for (i = 0; i < count_unknowns; i++)
	//{
